...
``` 
 - Подключиться к поду 
```shell
kubectl debug -ti nginx --image=perovmpr/alpine-strace-curl-tcpdump:0.0.1 --target=nginx
```
...
kubectl debug -ti nginx --image=perovmpr/alpine-strace-curl-tcpdump:0.0.1 --target=nginx -- curl 127.0.0.1
...
# Создать контейнер отладки с профилем у которого есть повышенные привилегии и подключиться к нему 
kubectl debug -ti nginx --image=perovmpr/alpine-strace-curl-tcpdump:0.0.1 --target=nginx --profile=general
# Находим pid процесса nginx
ps axu | grep nginx
    1 root      0:00 nginx: master process nginx -g daemon off;
    7 101       0:00 nginx: worker process
   73 root      0:00 grep nginx
# Запускаем strace
strace -p 7
...
# strace -p 7
strace: Process 7 attached
epoll_wait(8, [{events=EPOLLIN, data={u32=3685179408, u64=140685338959888}}], 512, -1) = 1
accept4(6, {sa_family=AF_INET, sin_port=htons(52760), sin_addr=inet_addr("127.0.0.1")}, [112 => 16], SOCK_NONBLOCK) = 3
epoll_ctl(8, EPOLL_CTL_ADD, 3, {events=EPOLLIN|EPOLLRDHUP|EPOLLET, data={u32=3685179872, u64=140685338960352}}) = 0
epoll_wait(8, [{events=EPOLLIN, data={u32=3685179872, u64=140685338960352}}], 512, 60000) = 1
...
write(5, "127.0.0.1 - - [05/Feb/2025:16:11"..., 90) = 90
close(11)                               = 0
setsockopt(3, SOL_TCP, TCP_NODELAY, [1], 4) = 0
epoll_wait(8, [{events=EPOLLIN|EPOLLRDHUP, data={u32=3685179872, u64=140685338960352}}], 512, 65000) = 1
